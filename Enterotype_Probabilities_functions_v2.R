...
# enterot_file = file sample-enterotype by Raes. e.g. /media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Sample-enterotype/V1V2_3e_sample-enterotype.csv
...
# OUTPUT
# data frame for all input samples in rows
# with columns= Sample, Enterotype (Raes enterotyping), Enterotype_Probatype_consist, Probatype (enterotyping by subsampling enterotype), 
# Probatype_filtered (filter out only enterotypes with quality > accuracy (0.9 po umolch)), Probability (probability of enterotype)


getStablEntSubsampl <- function(input_file, enterot_file, N=1000,clust.number=3,accuracy=0.9,noiserem=TRUE,locat='all'){
  library(cluster)
  library(clusterSim)
  ...
  # black_mark - number of times none of the enterotypes is Bacteroides and/or Prevotella
  black_mark <-0
  ...
  # print(paste0('permutation ',i))
    ...
    tmp_chimera <- paste(GetDrivers(obs.bet[1], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]],GetDrivers(obs.bet[2], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]],GetDrivers(obs.bet[3], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]],sep=';')
    ...
    if(GetDrivers(obs.bet[1], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Bacteroides'){
      ent_prob_matrix["g__Bacteroides",first_enterotype] <- ent_prob_matrix["g__Bacteroides",first_enterotype]+1
    } else if (GetDrivers(obs.bet[1], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Prevotella'){
    ...
    if(GetDrivers(obs.bet[2], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Bacteroides'){
      ent_prob_matrix["g__Bacteroides",second_enterotype] <- ent_prob_matrix["g__Bacteroides",second_enterotype]+1
    } else if (GetDrivers(obs.bet[2], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Prevotella'){
    ...
    if(GetDrivers(obs.bet[3], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Bacteroides'){
      ent_prob_matrix["g__Bacteroides",third_enterotype] <- ent_prob_matrix["g__Bacteroides",third_enterotype]+1
    } else if (GetDrivers(obs.bet[3], n=as.numeric(1))[[2]][[1]][[1]][[as.numeric(1)]] %like% 'g__Prevotella'){
  ...
  # Now make a table sample-enterotype: columns = Sample, Enterotype, Enterotype_Probatype_consist, Probatype, Probatype_filtered, Probability
  ...
  sample.enterotype <- matrix(0,dim(ent_prob_matrix)[2],6)
  colnames(sample.enterotype) <- c('Sample','Enterotype','Enterotype_Probatype_consist','Probatype','Probatype_filtered','Probability')
  #first, make a vector 'enterot_vector' from the enterot_file
  enterot_table <- read.table(enterot_file)
  factor1 <- as.factor(enterot_table[,2])
  if(length(levels(factor1))!=3){
    warning('in the input enterot_file the number of enterotypes not equal to 3! Enterotypes are here anyway classified as g__Bacteroides, g__Prevotella and enterotype_3')
  }
  enterot_vector <- c(rep('',times=length(rownames(enterot_table))))
  for (i in 1:length(enterot_vector)){
    if(enterot_table[i,2] %like% 'g__Bacteroides'){
      enterot_vector[i] <- 'g__Bacteroides'
    } else if(enterot_table[i,2] %like% 'g__Prevotella'){
      enterot_vector[i] <- 'g__Prevotella'
    } else{
      enterot_vector[i] <- 'enterotype_3'
    }
    names(enterot_vector)[i] <- as.character(enterot_table[i,1])
  }
  Enterotype_Probatype_consist <- c(rep(0),times=length(enterot_vector))
  if(!is.numeric(ent_prob_matrix)){ent_prob_matrix <- as.numeric(ent_prob_matrix)}
    for (i in 1:dim(ent_prob_matrix)[2]){
      enterotype <- enterot_vector[as.character(colnames(ent_prob_matrix)[i])]
      probatype <- which.max(ent_prob_matrix[1:3,i])
      if (ent_prob_matrix['total',i]==0){next()}
      probability <- signif(ent_prob_matrix[probatype,i]/ent_prob_matrix['total',i],digits = 3)
      if(probability >= accuracy){Probatype_filtered = names(probatype)}else{Probatype_filtered = NA}
      Enterotype_Probatype_consist <- as.numeric(enterotype==names(probatype))
      sample.enterotype[i,] <- c(colnames(ent_prob_matrix)[i],enterotype,Enterotype_Probatype_consist,names(probatype),Probatype_filtered,probability)
    }
  sample.enterotype <- as.data.frame(sample.enterotype)
  print(paste0('The number of iterations with no Bacteroides or no Prevotella enterotype: ',black_mark,' out of ',N))
...
# Probatype (=enterotype probab by subsampling) in a specific amplicon (say, v1v2, file1)
# enterotype probability obtained by getStablEntSubsampl function
# INPUT
# file1 = a table of enterotype probabilities done by subsampling ('Sample','Enterotype','Enterotype_Probatype_consist','Probatype','Probatype_filtered','Probability')
# file2 = sample-stable enterotype across locations
# folder = folder for drawing graph, suggested folder = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Enterotype_Probabilities/Graphs_enterotype_probability'
# graph.file = 'v1v2_ent_probability_by_sampl_vs_v12v34v56.jpg' - need to change depending on amplicon. If empty, no graph
...
subsVsLoc <- function(file1 = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Enterotype_Probabilities/V1V2_3e_sample-enterotype_probabilities.csv',file2='/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Sample-enterotype/Stable_enterotypes.csv', folder = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Enterotype_Probabilities/Graphs_enterotype_probability',graph.file1 = '',graph.file2='',window=200){
  ...
  # than delete samples where Enterotype (by Raes) differs from Probatype
  ent.with.prob <- ent.with.prob[ent.with.prob$Enterotype_Probatype_consist==1,]
  # sort the ent.with.prob according to probability of enterope
  ent.with.prob <- ent.with.prob[order(ent.with.prob$Probability,decreasing = T),]
  # make a vector of [number-of-samples] values 0 = absent in stable enterotypes, 1 = present
  ...
  for (i in 1:dim(ent.with.prob)[1]){
    if(ent.with.prob$Probability[i]==1){n=i}else{break}
  }
  n=n+1
  ...
  for (i in 1:length(identity.vector)){
    ent.stability.besttoworst[i] <- signif(mean(identity.vector[1:i]),digits = 3)
  }
  
  if(graph.file1!=''){
  ylab1 = paste0('Probability to be among stable enterotypes for the sample, window size ', window)
  jpeg(filename = graph.file1,width=1000,height = 600)
  plot(stability.window, xlab = 'Sample enterotype probability index in descend order', ylab = ylab1)
  dev.off()
  }
  if(graph.file2!=''){
  jpeg(filename = graph.file2,width = 1000, height = 600)
  plot(ent.stability.besttoworst[n:dim(ent.with.prob)[1]], ylim=c(0.5,0.8), xlab='Sampl sort enterotype probabil in descend, prob 100% excl', ylab='Probability of stable enterotype')
  dev.off()
  }
  return(list('stability.window'=stability.window,'identity.vector'=identity.vector,'ent.stability.besttoworst.sum.first.n.samples'=ent.stability.besttoworst,'first.probatype.less.1'=n))
}
  
# This program is to be run after subsVsLoc.
# It is input the file for Probatypes (probabilities of enterotypes) same that subsVsLoc, than finds Probability corresponding to
# the rank_cutoff given by user (after seing the graphic output of subsVsLoc: e.g. v1v2_ent_probability_by_sampl_vs_v12v34v56.jpg). 
# E.g., we see that samples after 863 (if sorted by Probability of Probatype) show huge descend in 
# enterotype stability across amplicons. So, we get that Probability cutoff is 95.4% e.g. 
# The samples with Probability < cutoff should be discarded.
getProbabilCutoff <- function(file,rank_cutoff,delete_unconsistent=TRUE){
  library(data.table)
  ent.with.prob <- read.table(file)
  if(delete_unconsistent){
  # than delete samples where Enterotype (by Raes) differs from Probatype
  ent.with.prob <- ent.with.prob[ent.with.prob$Enterotype_Probatype_consist==1,]
  }
  # sort the ent.with.prob according to probability of enterope
  ent.with.prob <- ent.with.prob[order(ent.with.prob$Probability,decreasing = T),]
  probabil.cutoff <- ent.with.prob$Probability[rank_cutoff]
  return(probabil.cutoff)
}