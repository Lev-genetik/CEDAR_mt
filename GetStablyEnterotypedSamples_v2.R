# v2. can work with both 2 and 3 enterotype groups
...
getStableSamples <- function(df1,df2,df3,correspondence,number.enterotypes){
   if((dim(df1)[2]!=2)|(dim(df2)[2]!=2)|(dim(df3)[2]!=2)){
   stop('number of columns in the input data frames should be 2!')
   geterrmessage()
   } 
  if(number.enterotypes!=2 & number.enterotypes!=3){
    stop('number of enterotypes should be 2 or 3!')
  }
  ...
  # check that df2 and df3 has all samples from df1. If no, work with the intersection of samples only
  intersection_123 <- intersect(intersect(df1[,1],df2[,1]),df3[,1])
  if((length(intersect(df1[,1],df2[,1]))!=dim(df1)[1])|(length(intersect(df1[,1],df3[,1]!=dim(df1)[1])))){
    ...
    df1 <- df1[df1[,1]%in%intersection_123,]
    df2 <- df2[df2[,1]%in%intersection_123,]
    df3 <- df3[df3[,1]%in%intersection_123,]
  }
  # add 3rd row to df1,df2 and df3: universal cluster number (1,2 or 3)
    ...
    # if we have only 2 enterotypes, df1.ent3 will be character(0)
    ...
    if(number.enterotypes==3){ent3.name <- paste(correspondence[3,],collapse='|')}
    ...
    # now get unstable samples
    unstable.samples <- intersection_123[!intersection_123 %in% stable.samples]
    # the result
     df.result <-cbind(stable.samples,enterotypes,enterotypes.short)
     return(list(df.result,unstable.samples))
}