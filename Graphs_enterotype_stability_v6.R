...
# INPUT:
# outliers.removed = TRUE if outliers removed while enterotyping,  => we need to extract only samples in common across files
# podrovniat_files = TRUE if the set of samples is not the same exactly across amplicons.
# if outliers.removed=TRUE than automatically podrovniat_files = TRUE
...
source('/home/lev-genetik/Desktop/Projects/liege/src/Lev/Enterotyping_Raes_noise_removed_v11.R')
...
if (length(levels(sample.enterotype[,2])[levels(sample.enterotype[,2])%like%'g__Bacteroides'])!=0){
  ...
  ##
  # this is a temprorary solution
  if(length(ent_instead_Bact)==0){
    warning('achtung! problem with Bacteroides enterotype assignment. First matching enterotype made e1')
    ent_instead_Bact <- levels(sample.enterotype[,2])[1]
  }
  # end of temprorary solution
  ##
...
levels(sample.enterotype[,2])[levels(sample.enterotype[,2])%like%'g__Prevotella'] <- 'P'
  ...
  ##
  # this is a temprorary solution
  if(length(ent_instead_Prev)==0){
    warning('achtung! problem with Prevotella enterotype assignment. First matching enterotype made e1')
    ent_instead_Prev <- levels(sample.enterotype[,2])[2]
  }
  # end of temprorary solution
  ##
