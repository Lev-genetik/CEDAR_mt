...
# VERY STRANGE... THIS PROGRAM IS COMMENTED AS IT DOES NOT WORK. NEED TO FINALIZE IT 26.12.18
# NB in .v4 there is a strange problem: stable.three used both as logical (take samples stable in all 3 loc=TRUE)
# and as a data frame with these samples. Tries to solve in .v5, let's see
...
#   result <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
#   stable.total <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
#   result.stable.three <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
#   result.unstable.three <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
#   result.stable.two <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
#   result.unstable.two <- data.frame("stable.samples"=character(),"enterotypes"=character(),"enterotypes.short"=character())
...
#   if(stable.three){result <- rbind(result,stability.across.ampl$stable.v12v34v56)}
...
# 
#   if(unstable.two){
#     result.unstable.two <- rbind(stability.across.ampl$unstable.v12v34,stability.across.ampl$unstable.v12v56,
#                                  stability.across.ampl$unstable.v34v56)
#   }
#   
# }
