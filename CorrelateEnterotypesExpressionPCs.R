...
# Output: 
# a matrix with info of normality and variance equality for each PC.
...
corrEnterExpr.initiation <- function(file1='/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Sample-enterotype/Stable_enterotypes.csv',file2,locat,n=10){
 ...
 # check that dimensions of files are nx3 and mxm+2
  ...
  if (locat=='IL'){
    ...
    enterotypes <- enterotypes[enterotypes[,1] %like% '\\.all',]
    enterotypes[,1] <- gsub('\\.all','',enterotypes[,1])
    
  }else {
    stop()
    geterrmessage('locat must be IL, TR or RE')
  }
  ...
  # the easiest part done. The most difficult: supplement by the enterotypes got for sample.1,.2,.3
  # if the original sample does not have stable enterotype.
  # first, delete .1,.2 and .3 from enterotypes$stable.samples
  enterotypes.stable.samples.short <- gsub('\\.[1-3]','',enterotypes$stable.samples)
  ...
  # get p-values for shapiro test for normality of PC i-2 for each enterotype
    ...
    shapiro3 <- shapiro.test(mod.PCs[mod.PCs[,1]=='enterotype_3',i])$p.value
    ...
    var13 <- var.test(mod.PCs[mod.PCs[,1]=='Bacteroides',i],mod.PCs[mod.PCs[,1]=='enterotype_3',i])$p.value
    var23 <- var.test(mod.PCs[mod.PCs[,1]=='Prevotella',i],mod.PCs[mod.PCs[,1]=='enterotype_3',i])$p.value
    normaliok[i-2,1] <- min(shapiro1,shapiro2,shapiro3)
    normaliok[i-2,2] <- min(var12,var13,var23)
...
corrEnterExpr.test <- function(file_1, file_2,locat_,PC.number=1,test.type = 'wilcox'){
  i <- PC.number + 2
  test.p.val <- numeric(3)
  sample.size <- integer(3)
  average <- numeric(3)
  st.dev <- numeric(3)
  ...
  test.p.val[3] <- wilcox.test(PCs[PCs[,1]=='Prevotella',i],PCs[PCs[,1]=='enterotype_3',i])$p.value
    ...
    stop()
    geterrmessage('test.type must be wilcox or t.test')
  }
  ...
  rownames(additional.info) <- c('Bacteroides','Prevotella','Enterotype_3')
...
corrEnterExpr.test.n <- function(file_1=file_1, file_2=file_2,locat_=locat_,n=3,test.type = 'wilcox'){
  associat <- list()
  for (i in 1:n){
  ...
  for (i in 2:n){
    associat.pvalues <- cbind(associat.pvalues,as.data.frame(associat[[i]]$Mann.Whitney.pvalues))
  }
  
  # Krivivato, no net zhelaniya pravit'
  names.tmp <- c('PC1.Mann.Whit.pval','PC2.Mann.Whit.pval','PC3.Mann.Whit.pval','PC4.Mann.Whit.pval','PC5.Mann.Whit.pval','PC6.Mann.Whit.pval','PC7.Mann.Whit.pval','PC8.Mann.Whit.pval','PC9.Mann.Whit.pval','PC10.Mann.Whit.pval')

  names(associat.pvalues) <- names.tmp[1:n]
