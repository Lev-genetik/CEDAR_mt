...
Enterotyping_Raes <- function (input_file, output_dir, clust.number = 4, noiserem=T, locat = 'all', driv.number = 10, plots=F) {
library(cluster)
library(clusterSim)
...
names(result) <- c('drivers_n','sample.enterotype','drivers_unique','obs_bet_result')
return(result)
}