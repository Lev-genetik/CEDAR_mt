...
Enterotyping_Raes <- function (input_file, output_dir, clust.number = 4, noiserem=T, locat = 'all', plots=F) {
library(cluster)
library(clusterSim)
...
# if(anyDuplicated(clusters[,1])){
#   i=2
#   while((anyDuplicated(clusters[,1]))&(i<=ncol(clusters))){
#     for (t in 1:nrow(clusters)){
#       clusters[t,1] <- paste(clusters[t,1],clusters[t,i],sep='+')
#     }    
#   }
# }

result <- list(clusters,sample.enterotype)
return(result)
}