...
zeros <- rep(0, length(colnames(data))*2*7)
sample.enterotype <- array(zeros, c(length(colnames(data)), 2, 7))
for (i in 1:length(colnames(data))){
  sample.enterotype[i,1,1] <- colnames(data[i])
  sample.enterotype[i,2,1] <- data.cluster[i]
  #instead of number need driver #1
  # paste(str_remove(str_remove(str_remove(GetDrivers(obs.bet[1], n=1),'..__$'), '..__$'), '.*__'), collapse = '|')
}

jpeg(filename = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Clustering/With_noise_removal/Genus_level/CH_Opt_number_clust_for_L6_new.jpg')
plot(nclusters, type="h", xlab="Number of clusters", ylab="CH index",main="Calinski-Harabasz pseudo F-statistic", lwd=2, cex.lab=1)
dev.off()
jpeg(filename = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Clustering/With_noise_removal/Genus_level/Silhouettes_Opt_number_clust_for_L6_new.jpg')
...
# dev.new()
#finished - colors
jpeg(filename = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Clustering/With_noise_removal/Genus_level/Between_class_anal_2_eigenvectors_PCA.jpg', width=2048, height=2048)
...
jpeg(filename = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Clustering/With_noise_removal/Genus_level/Between_class_PCOA.jpg', width=2048, height=2048)
s.class(obs.pcoa$li, fac=as.factor(data.cluster), col=c('red','orange','green','blue'), grid=F,sub="Principal coordiante analysis")
dev.off()
jpeg(filename = '/media/lev-genetik/980E73270E72FD96/Liege/Results/Clustering/With_noise_removal/Genus_level/Between_class_PCOA_3D.jpg', width=640, height=640)
