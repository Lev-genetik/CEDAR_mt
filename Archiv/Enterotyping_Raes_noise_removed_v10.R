# v10 changes: now removes outliers (1% of most distant samples)
# v9 changes: 
...
# input arg remove.outliers: if not removing outliers, must be = 0. Otherwise if = 0.01 (po umolch) - remove 
...
Enterotyping_Raes <- function (input_file, output_dir, clust.number = 4, noiserem=T, locat = 'all', remove.outliers = 0, driv.number = 10, plots=F) {
library(cluster)
library(clusterSim)
...
result <- list(clusters,sample.enterotype,drivers_unique,obs.bet,drops.new)
names(result) <- c('drivers_n','sample.enterotype','drivers_unique','obs_bet_result','outliers')
return(result)
}