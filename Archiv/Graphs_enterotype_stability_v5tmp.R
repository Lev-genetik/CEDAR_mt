...
ylim = c(min(ent.Raes.3e.curr.locat$obs_bet_result$ls[,2])-1,max(ent.Raes.3e.curr.locat$obs_bet_result$ls[,2])+1)
lwd = 10
s.class(ent.Raes.3e.curr.locat$obs_bet_result$ls[stable.locat.12_34_56_numbers,], fac=as.factor(sample.enterotype$enterotype[stable.locat.12_34_56_numbers]), col=c('red','red','red','red'), xlim=xlim, ylim=ylim, cpoint = 2, grid=F, sub=legend, cstar = 0, csub = 4, cellipse = 0)
