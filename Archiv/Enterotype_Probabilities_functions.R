...
getStablEntSubsampl <- function(input_file,N=1000,clust.number=3,accuracy=0.9,noiserem=TRUE,locat='all'){
  library(cluster)
  library(clusterSim)
  ...
  colnames(sample.enterotype) <- c('Sample','Enterotype','Stable_enterotype','Probability')
...
# enterotype probab by subsampling in a specific amplicon (say, v1v2, file1)
# enterotype probability obtained by getStablEntSubsampl function
# INPUT
# file1 = a table of enterotype probabilities done by subsampling ('Sample','Enterotype','Stable_enterotype','Probability')
