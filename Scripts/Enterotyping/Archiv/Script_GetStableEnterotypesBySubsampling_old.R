...
# now draw graphs
library(graphics)
setwd('/media/lev-genetik/980E73270E72FD96/Liege/Results/Enterotyping/2018_09_06_enterotyping/Sample-enterotype/Enterotype_Probabilities/Graphs_enterotype_probability')

jpeg(filename = 'Ent_stabil_density_12_black_34_red_56_blue_095_100.jpg',width=1000,height = 600)
plot(density(sample.enterotype.stable.subsampl.v12$probability),xlim = c(0.95,1),xlab = 'Enterotype probability',main = '')
lines(density(sample.enterotype.stable.subsampl.v34$probability), col='red')
lines(density(sample.enterotype.stable.subsampl.v56$probability), col='blue')
dev.off()

jpeg(filename = 'Ent_stabil_density_12_black_34_red_56_blue_050_100.jpg',width=1000,height = 600)
plot(density(sample.enterotype.stable.subsampl.v12$probability),xlim = c(0.5,1),xlab = 'Enterotype probability',main = '')
lines(density(sample.enterotype.stable.subsampl.v34$probability), col='red')
lines(density(sample.enterotype.stable.subsampl.v56$probability), col='blue')
dev.off()

jpeg(filename = 'Ent_stabil_density_v1v2_0_1.jpg',width=1000,height = 600)
plot(density(sample.enterotype.stable.subsampl.v34$probability),xlim = c(0,1),xlab = 'v3v4 enterotype probability',col='red',main = '')
dev.off()
jpeg(filename = 'Ent_stabil_density_v3v4_0_1.jpg',width=1000,height = 600)
plot(density(sample.enterotype.stable.subsampl.v12$probability),xlim = c(0,1),xlab = 'v1v2 enterotype probability',col='black',main = '')
dev.off()
jpeg(filename = 'Ent_stabil_density_v5v6_0_1.jpg',width=1000,height = 600)
plot(density(sample.enterotype.stable.subsampl.v56$probability),xlim = c(0,1),xlab = 'v5v6 enterotype probability',col='blue',main = '')
dev.off()
...
sample.enterotype.stable.subsampl.v12.sorted <- sample.enterotype.stable.subsampl.v12[order(-sample.enterotype.stable.subsampl.v12$probability),]
for (i in 1:dim(sample.enterotype.stable.subsampl.v12.sorted)[1]){
  if(sample.enterotype.stable.subsampl.v12.sorted$probability[i]==1){n=i}else{return()}
}
jpeg('Enterotype_probabil_v12_descend_cumulat_vs_ent_probability_v12_v34_v56.jpg',width = 1000, height = 600)
plot(ent.prob.vs.stabililty.v12$ent.probability.besttoworst.sum.first.n.samples[n:dim(sample.enterotype.stable.subsampl.v12.sorted)[1]], ylim=c(0.5,0.75), xlab='Sampl sort enterotype probabil in descend, prob 100% excl', ylab='Probability of stable enterotype')
dev.off()
