...
getStabStats <- function(df1){
  ...
  # get sample numbers. 
  sample.numbers.IPC <- gsub('\\.IL|\\.TR|\\.RE','',df1[,1])
  sample.numbers <- gsub('IPC','',sample.numbers.IPC)
  first.occurence <- !duplicated(sample.numbers)
...
# get enterotypes for all samples that have info for 2 locations: e.g. row 1 = sample 1, colons are locations IL|TR|RE
  ...
  for (i in 1:length(samples.unique)){
    if (!is.null(df1[sample.numbers.IPC %like% samples.unique[i],])&(!is.vector(df1[sample.numbers.IPC %like% samples.unique[i],]))){
    if(dim(df1[sample.numbers.IPC %like% samples.unique[i],])[1]==3){
     m <- as.vector(df1[sample.numbers.IPC %like% samples.unique[i],2])
     result.3 <- rbind(result.3,m)
     rownames(result.3)[count.3]=samples.unique[i]
     count.3 <- count.3 + 1
    }
     else if(dim(df1[sample.numbers.IPC %like% samples.unique[i],])[1]==2){
     m <- as.vector(df1[sample.numbers.IPC %like% samples.unique[i],2])
